<template>
  ...
  infinite-scroll-distance="3">
      <li  v-for="item in comment" :key="item.id" class="mui-table-view-cell mui-media">
...
<script>
import {bookcomment} from '../../network/request' 
...
watch:{
 ...
 created(){
    ...
    methods:{
        loadMore(){
            bookcomment(this.$route.params.id,this.limit).then(res => {
        if (res.status === 200) {
